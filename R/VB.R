DLSBP_VB_univ <- function(y, X, H, prior, maxiter, tol, method_init, verbose, verbose_step) {
  ...
  B_mixing <- prior$B_mixing; P_mixing <- solve(B_mixing); Pb_mixing <- P_mixing %*% b_mixing; eig_B <- eigen(B_mixing); logdetB <- sum(log(eig_B$values))
  ...
  if(method_init=="cluster") {
    if(verbose) cat("Clustering observation before starting VB algorithm...\n") 
    cluster <- clara(X,H)$clustering 
    z       <- model.matrix(y~as.factor(cluster)-1); z <- z + 1e-8; z <- z/rowSums(z)
    for(h in 1:(H-1)) {rho[,h]     <- z[,h]/rowSums(z[, h:H])}
  } else if(method_init=="EM"){
    out_EM <- DLSBP_EM_univ(y=y, X=X, H=H, prior=prior, 
                  maxiter=maxiter, tol=tol, method_init="cluster",
                  verbose=verbose, 
                  verbose_step=300)
    z       <- out_EM$z; z <- z + 1e-8; z <- z/rowSums(z)
    for(h in 1:(H-1)) {rho[,h]     <- z[,h]/rowSums(z[, h:H])}
  } else {
    if(verbose) cat("Random initialization of the VB algorithm...\n")
    # Random initialization
    mu_mixing    <- matrix(rnorm((H-1)*p,-.1,0.1), H - 1, p) 
    Sigma_mixing <- array(0,c(H-1,p,p))
  ...
  for (r in 1:maxiter) {
    ...
    for (h in 1:H) {
      ...
      if (h < H) {
        ...
        lower5[h]  <-  -p*log2pi/2 - determinant(Sigma_mixing[h,,],logarithm=TRUE)$modulus/2 - p/2
  ...
  list(param=list(mu_mixing = mu_mixing, Sigma_mixing=Sigma_mixing, mu_kernel = mu_tilde, Sigma_kernel=1/tau_tilde, a_tilde=a_tilde, b_tilde=b_tilde), cluster=cluster, z=z, lowerbound=lowerbound)
}
...
DLSBP_VB_multi <- function(y, X1, X2, H, prior, maxiter, tol, method_init, verbose, verbose_step) {
  ...
  B_mixing <- prior$B_mixing; P_mixing <- solve(B_mixing); Pb_mixing <- P_mixing %*% b_mixing; eig_B <- eigen(B_mixing); logdetB <- sum(log(eig_B$values))
  B_kernel<- prior$B_kernel; P_kernel <- solve(B_kernel); 
  Pb_kernel <- P_kernel %*% b_kernel;eig_Bkernel <- eigen(B_kernel);logdetB_kernel <- sum(log(eig_Bkernel$values))
  a_tau <- prior$a_tau
  b_tau <- prior$b_tau
  ...
  if(method_init=="cluster") {
    ...
    out_EM <- DLSBP_EM_multi(y=y,X1=X1, X2=X2, H=H, prior=prior, 
                            maxiter=maxiter, tol=tol, method_init="cluster",
                            verbose=verbose, 
                            verbose_step=300)
    z       <- out_EM$z; z <- z + 1e-8; z <- z/rowSums(z)
    ...
    # Random initialization
    mu_mixing    <- matrix(rnorm((H-1)*p_mixing,-.1,0.1), H - 1, p_mixing) 
    Sigma_mixing <- array(0,c(H-1,p_mixing,p_mixing))
  ...
  for (r in 1:maxiter) {
    for (h in 1:H) {
      ...
      residual[,h]      <- y^2 - 2*y*linpred_kernel[,h]  +  linpred_kernel[,h]^2 + rowSums(X1 %*% Sigma_kernel[h,,] * X1) # E( (y - xkernel)^2) = y^2 - 2*y*E(eta) + E(eta^2)
      a_tilde[h]   <- a_tau  + sum(z[, h])/2
      b_tilde[h]   <- b_tau + sum(z[, h] * residual[,h])/2
      tau[h]       <- a_tilde[h] / b_tilde[h]
      ltau[h]      <- digamma(a_tilde[h]) - log(b_tilde[h])
    ...
    for (h in 1:H) {
      lower1[h]  <-  sum(z[,h]*(ltau[h]/2 - log2pi/2 - tau[h]/2*residual[,h]))
      lower4[h]  <- -p_kernel*log2pi/2 - logdetB_kernel/2 - 0.5*(sum(diag(P_kernel%*%Sigma_kernel[h,,])) + t(mu_kernel[h,] - b_kernel)%*%P_kernel%*%(mu_kernel[h,] - b_kernel)) + a_tau*log(b_tau) - lgamma(a_tau) - b_tau*tau[h] + (a_tau-1)*ltau[h]
      lower6[h]  <-  -p_kernel*log2pi/2 - determinant(Sigma_kernel[h,,],logarithm=TRUE)$modulus/2 - p_kernel/2 + a_tilde[h]*log(b_tilde[h]) - lgamma(a_tilde[h]) - b_tilde[h]*tau[h] + (a_tilde[h]-1)*ltau[h]
      
      if (h < H) {
        lower2[h]  <-  sum(rho[,h]*linpred_mixing[,h] - (linpred_mixing[,h] + xi[,h])/2 + log(plogis(xi[,h])))
        ...
        lower5[h]  <-  -p_mixing*log2pi/2 - determinant(Sigma_mixing[h,,],logarithm=TRUE)$modulus/2 - p_mixing/2
        lower7[h]  <-  sum(rho[,h]*log(rho[,h]) + (1 - rho[,h])*log(1-rho[,h]))
      }
    }
    # Convergence checks
  ...
  # Output
  cluster <- apply(z,1,which.max)
  list(param=list(mu_mixing = mu_mixing, Sigma_mixing=Sigma_mixing, mu_kernel = mu_kernel, Sigma_kernel=Sigma_kernel, a_tilde=a_tilde, b_tilde=b_tilde), cluster=cluster, z=z, lowerbound=lowerbound)
}