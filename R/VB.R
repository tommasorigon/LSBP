DLSBP_VB_univ <- function(y, X, H, prior, maxiter, tol, method_init, verbose, verbose_step) {
      ...
      for (h in 1:H) {
         ...
         if (h < H) {
            lower2[h] <- sum(rho[, h] * linpred[, h] - (linpred[, h] + xi[, h])/2 + log(plogis(xi[, 
              h])))
            lower3[h] <- -p * log2pi/2 - logdetB/2 - 0.5 * (sum(diag(P_mixing %*% Sigma_mixing[h, 
              , ])) + t(mu_mixing[h, ] - b_mixing) %*% P_mixing %*% (mu_mixing[h, ] - b_mixing))
   ...
   list(param = list(mu_mixing = mu_mixing, Sigma_mixing = Sigma_mixing, mu_kernel = mu_tilde, Sigma_kernel = 1/tau_tilde, 
...
DLSBP_VB_multi <- function(y, X1, X2, H, prior, maxiter, tol, method_init, verbose, verbose_step) {
   ...
   for (r in 1:maxiter) {
      ...
      for (h in 1:H) {
         lower1[h] <- sum(z[, h] * (ltau[h]/2 - log2pi/2 - tau[h]/2 * residual[, h]))
         lower4[h] <- -p_kernel * log2pi/2 - logdetB_kernel/2 - 0.5 * (sum(diag(P_kernel %*% Sigma_kernel[h, 
            , ])) + t(mu_kernel[h, ] - b_kernel) %*% P_kernel %*% (mu_kernel[h, ] - b_kernel)) + 
         ...
         if (h < H) {
            lower2[h] <- sum(rho[, h] * linpred_mixing[, h] - (linpred_mixing[, h] + xi[, h])/2 + 
              log(plogis(xi[, h])))
            lower3[h] <- -p_mixing * log2pi/2 - logdetB/2 - 0.5 * (sum(diag(P_mixing %*% Sigma_mixing[h, 
              , ])) + t(mu_mixing[h, ] - b_mixing) %*% P_mixing %*% (mu_mixing[h, ] - b_mixing))
   ...
   # Output
   cluster <- apply(z, 1, which.max)
   list(param = list(mu_mixing = mu_mixing, Sigma_mixing = Sigma_mixing, mu_kernel = mu_kernel, Sigma_kernel = Sigma_kernel, 
      a_tilde = a_tilde, b_tilde = b_tilde), cluster = cluster, z = z, lowerbound = lowerbound)
}