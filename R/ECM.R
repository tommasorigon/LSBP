LSBP_ECM_multi <- function(y, X1, X2, H, prior, maxiter, tol, method_init, verbose, verbose_step) {
   ...
   for (r in 1:maxiter) {
      ...
      logpriors <- sum(ldmvnorm(beta_mixing, b_mixing, eig_Bmixing)) + sum(ldmvnorm(beta_kernel, b_kernel, 
         eig_Bmixingkernel)) + sum(dgamma(tau, a_tau, b_tau, log = TRUE))
   ...
   out <- list(param = list(beta_mixing = beta_mixing, beta_kernel = beta_kernel, tau = tau), cluster = cluster, 
...
LSBP_ECM_univ <- function(y, X, H, prior, maxiter, tol, method_init, verbose, verbose_step) {
   ...
   # Output
   cluster <- apply(z, 1, which.max)
   out <- list(param = list(beta_mixing = beta, beta_kernel = mu, tau = tau), cluster = cluster, z = z, 
