DLSBP_Gibbs_univ <- function(y, X, prior, H, R, burn_in, method_init, verbose, verbose_step){
  ...
  B_mixing      <- prior$B_mixing; P_mixing <- solve(B_mixing); Pb <- P_mixing %*% b_mixing; eig_B <- eigen(B_mixing)
  ...
  if(method_init=="cluster") {
    if(verbose) cat("Clustering observation before starting Gibbs sampling...\n")
  ...
  # Gibbs sampling
  if(verbose) cat(paste("Starting the Gibbs sampling with R = ",R," and burn_in = ", burn_in,". \n",sep=""))
  for (r in 1:(R + burn_in)) {
    ...
    logpriors      <- sum(ldmvnorm(beta_mixing, b_mixing,eig_B)) + sum(dnorm(mu,  mu_mu,1/sqrt(tau_mu),log=TRUE)) + sum(dgamma(tau, a_tau,b_tau,log=TRUE))
    ...
    if (verbose) {
      if(r%%verbose_step==0) cat(paste("Sampling iteration: ", r, ", logposterior: ",round(logpost[r],4),".\n", sep=""))
    }
  }
  list(logposterior=logpost, param=list(beta_mixing=beta_mixing_out, beta_kernel=mu_out, tau=tau_out))
}

DLSBP_Gibbs_multi <- function(y, X1, X2, prior, H, R, burn_in, method_init, verbose, verbose_step){
  ...
  B_mixing <- prior$B_mixing; P_mixing <- solve(B_mixing); Pb_mixing <- P_mixing %*% b_mixing; eig_B <- eigen(B_mixing)
  B_kernel<- prior$B_kernel; P_kernel <- solve(B_kernel); Pb_kernel <- P_kernel %*% b_kernel; eig_Bkernel <- eigen(B_kernel)
  a_tau <- prior$a_tau
  b_tau <- prior$b_tau
  ...
  for (r in 1:(R + burn_in)) {
    ...
    # Convergence checks
    loglik         <- Update$loglik
    logpriors      <- sum(ldmvnorm(beta_mixing, b_mixing,eig_B)) + sum(ldmvnorm(beta_kernel, b_kernel, eig_Bkernel)) + sum(dgamma(tau, a_tau,b_tau,log=TRUE))
    logpost[r]        <-   loglik + logpriors
  ...
  list(logposterior=logpost, param=list(beta_mixing=beta_mixing_out, beta_kernel=beta_kernel_out, tau=tau_out))
}
