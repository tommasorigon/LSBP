...
List G_pois_update(arma::vec y, arma::mat X, arma::mat beta, arma::vec tau){
  ...
  for(int i=0; i < n; i++) { 
    ...
    mean = tau[0];
    lprob[0]    = log(pi[0]) + R::dpois(y(i),mean,TRUE);
    ...
    for(int h = 1; h < H - 1; h++) {
      ...
      mean = tau[h];
      lprob[h] = log(pi[h]) + R::dpois(y(i),mean,TRUE);
    }
    ...
    mean = tau[H-1];
    lprob[H-1]    = log(pi[H-1]) + R::dpois(y(i),mean,TRUE);
...
List Variational_step_pois(arma::mat rho, arma::mat linpred, arma::vec y, arma::vec tau, arma::vec ltau)
{
  ...
  for(int i=0; i < n; i++) { 
    ...
    for(int h = 0; h < H - 1; h++) {
      ...
      // Linear predictor
      eta      = linpred(i,h) + sum(zl.row(i).t() % (y(i)*ltau  - tau));
      rho(i,h) = 1/(1+ exp(-eta));
...
arma::vec LSBP_density_Pois_C(double y, arma::mat X, arma::mat beta, arma::vec tau){
  ...
  for(int i=0; i < n; i++) { 
    ...
    pi[0]     = nu[0];
    cum_nu[0] = 1-nu[0];
    pdf[i]    = pi[0]*R::dpois(y, tau[0],FALSE);
    ...
    for(int h = 1; h < H - 1; h++) {
      ...
      pi[h]      = nu[h] * cum_nu[h-1];
      cum_nu[h]  = (1-nu[h])*cum_nu[h-1];
      pdf[i]     = pdf[i] + pi[h]*R::dpois(y, tau[h],FALSE);
    }
    
    // Last component
    nu[H-1]  = 1; // Set it equal to 1.
    pi[H-1]  = nu[H-1] * cum_nu[H-2];
    pdf[i]   = pdf[i] + pi[H-1]*R::dpois(y, tau[H-1],FALSE);
  }
